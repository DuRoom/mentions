...
import PostPreview from 'duroom/forum/components/PostPreview';
  ...
  extend(CommentPost.prototype, 'footerItems', function (items) {
    ...
    if (replies && replies.length) {
      ...
      const names = repliers.slice(0, overLimit ? limit - 1 : limit).map((reply) => {
        ...
        return (
          <Link href={app.route.post(reply)} onclick={hidePreview.bind(this)} data-number={reply.number()}>
            {app.session.user === user ? app.translator.trans('duroom-mentions.forum.post.you_text') : username(user)}
          </Link>
        );
      });
      ...
      if (overLimit) {
        ...
        names.push(app.translator.trans('duroom-mentions.forum.post.others_text', { count }));
      }
      ...
      items.add(
        ...
        <div className="Post-mentionedBy">
          <span className="Post-mentionedBy-summary">
            {icon('fas fa-reply')}
            {app.translator.trans('duroom-mentions.forum.post.mentioned_by' + (repliers[0].user() === app.session.user ? '_self' : '') + '_text', {
              count: names.length,
